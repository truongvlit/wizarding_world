...
public class ShowDetailController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Product product = client.find_XML(Product.class, id);
            if (product != null) {
                request.setAttribute("INFO", product);
                url = SUCCESS;
            } else {
                request.setAttribute("ERROR", "Sorry! The product doesn't exist");
            }
        } catch (Exception e) {
            log("ERROR at ShowDetailController: " + e.getMessage());
        } finally {
